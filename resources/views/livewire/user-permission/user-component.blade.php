            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <section>
                        ...
                        @endif
                        <div class="my-4">
                            <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" wire:click="create">Add User</button>
                        </div>
                        @if($isOpen)
                            <div class="fixed inset-0 flex items-center justify-center z-50">
                                ...
                                <div class="relative bg-gray-200 p-8 rounded shadow-lg w-1/2">
                                    ...
                                    <h2 class="text-2xl font-bold mb-4">{{ $userId ? 'Edit Post' : 'Create Post' }}</h2>
                                    {{--                                    <form wire:submit="store">--}}
                                    <form wire:submit.prevent="{{ $userId ? 'update' : 'store' }}">
                                        <div class="mb-4">

                                            <label for="name" class="block text-gray-700 font-bold mb-2">Nome:</label>
                                            {{--                                            <input type="text" id="name" class="w-full border border-gray-300 px-4 py-2 rounded">--}}
                                            <input type="text" wire:model="name" id="name" class="w-full border border-gray-300 px-4 py-2 rounded" autocomplete="off">
                                            <span class="text-red-500">@error('name') {{ $message }} @enderror</span>
                                        </div>
                                        <div class="mb-4">
                                            <label for="email" class="block text-gray-700 font-bold mb-2">EMail:</label>
                                            <input type="email" wire:model="email" id="email" class="w-full border border-gray-300 px-4 py-2 rounded" autocomplete="off">
                                            <span class="text-red-500">@error('email') {{ $message }} @enderror</span>
                                            {{--                                            <textarea id="email" rows="4" class="w-full border border-gray-300 px-4 py-2 rounded"></textarea>--}}
                                        </div>
                                        <div class="flex justify-end">

                                            <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded mr-2">{{ $userId ? 'Update' : 'Create' }}</button>
